...
namespace LotG.Battle
{
    public class BattleSystem : MonoBehaviour
    {
        ...
        private const string LOSE_MSG = "You lost the battle...";
        private const string FLEE_SUCCESS_MSG = "Your party fled the battle...";
        private const string FLEE_UNSUCCESS_MSG = "Your party couldn't flee the battle...";
        private const string OVERWORLD_SCENE = "Overworld_Scene";
                ...
                if (currentState == BattleState.Battle)
                {
                    switch (allCombatants[i].BattleAction)
                    {
                        case BattleEntity.Action.Feed:
                            yield return StartCoroutine(BattleRoutine(i));
                            break;
        ...
        private IEnumerator BattleRoutine(int i)
        {
            ...
            if (i < allCombatants.Count && allCombatants[i].IsPlayer == false)
            {
                BattleEntity currAttacker = allCombatants[i];
                if (!allCombatants[currAttacker.Target].IsPlayer || currAttacker.Target >= allCombatants.Count)
                {
                ...
                {
                    battleText.text = string.Format("{0} has been defeated!", currTarget.Name);
                    yield return new WaitForSeconds(TURN_DURATION);
                    //currTarget.BattleVisual.PlayDeathAnim();
                    playerCombatants.Remove(currTarget);
                    allCombatants.Remove(currTarget);
        ...
        private IEnumerator FleeRoutine()
        {
            if (currentState == BattleState.Battle)
            {
                ...
                {
                    battleText.text = FLEE_SUCCESS_MSG;
                    currentState = BattleState.Fled;
                    allCombatants.Clear();
                    yield return new WaitForSeconds(TURN_DURATION);
                    SceneManager.LoadScene(OVERWORLD_SCENE);
                    yield break;
                ...
                {
                    battleText.text = FLEE_UNSUCCESS_MSG;
                    yield return new WaitForSeconds(TURN_DURATION);
                }
            }
        }
        ...
        private void CreatePartyEntities()
        {
            ...
            for (int i = 0; i < partyMembers.Count; i++)
            {
                ...
                tempBattleVis.SetStartingValues(partyMembers[i].CurrHealth, partyMembers[i].MaxHealth,
        ...
        public void SelectFleeAction()
        {
            currentState = BattleState.Selection;

            BattleEntity currentPlayerEntity = playerCombatants[currentPlayer];
            currentPlayerEntity.BattleAction = BattleEntity.Action.Flee;
        ...
        private void AttackAction(BattleEntity currAttacker, BattleEntity currTarget)
        {
            ...
            currTarget.BattleVisual.PlayHitAnim();
            currTarget.UpdateHealthUI();
            battleText.text = string.Format("{0} attacked {1} for {2} damage!", currAttacker.Name, currTarget.Name, damage);
            SaveHealth();
        }
        ...
        private void SaveHealth()
        {
            ...
            {
                partyManager.SaveHealth(i, playerCombatants[i].CurrHealth);
            }
        }

        private void DetermineBattleOrder()
        {
            //Sorts combatants by initiative, highest to lowest
            allCombatants.Sort((bi1, bi2) => -bi1.Initiative.CompareTo(bi2.Initiative));
        }
    }
